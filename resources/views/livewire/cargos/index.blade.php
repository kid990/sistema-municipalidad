<section x-data="clientTableFilter()" class="flex h-full w-full flex-1 flex-col gap-4">
    ...
    <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
        <h1 class="text-2xl font-bold">{{ __('Cargos') }}</h1>
        <p class="text-neutral-600 dark:text-neutral-400 mt-2">{{ __('Gestiona los cargos del sistema') }}</p>
    </div>

    <!-- Lista y Botón -->
    <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-semibold">{{ __('Lista de Cargos') }}</h2>
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr x-show="matches($el.dataset.filter)"
                        x-cloak
                        data-filter="{{ \Illuminate\Support\Str::lower($cargo->nombre_cargo) }}"
                        class="border-b border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-800">
                        <td class="py-3 px-4">{{ $cargo->nombre_cargo }}</td>
                        <td class="py-3 px-4">
                            <div class="flex gap-2">
                                <button wire:click="openEdit({{ $cargo->id }})"
                                        type="button"
                                        title="{{ __('Editar') }}"
                    ...
                    @empty
                    <tr class="border-b border-neutral-200 dark:border-neutral-700">
                        <td colspan="2" class="py-8 px-4 text-center text-neutral-500">
                            {{ __('No hay registros') }}
                        </td>
                    </tr>
                    @endforelse
        ...
        <div wire:click.stop
            ...
            <div class="flex items-center justify-between mb-4">
                <h3 id="modal-title" class="text-xl font-bold">
                    {{ $editingId ? __('Editar Cargo') : __('Nuevo Cargo') }}
                </h3>
            ...
            <form wire:submit="save" class="space-y-4">
                <div>
                    <label for="nombre_cargo" class="block text-sm font-medium mb-2">{{ __('Nombre') }}</label>
                    <input type="text"
                           id="nombre_cargo"
                           wire:model="form.nombre_cargo"
