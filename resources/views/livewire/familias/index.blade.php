<section x-data="clientTableFilter()" class="flex h-full w-full flex-1 flex-col gap-4">
    ...
    <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
        <h1 class="text-2xl font-bold">{{ __('Familias') }}</h1>
        <p class="text-neutral-600 dark:text-neutral-400 mt-2">{{ __('Gestiona las familias del sistema') }}</p>
    </div>

    <!-- Lista y Boton -->
    <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-semibold">{{ __('Lista de Familias') }}</h2>
        ...
        <div class="mb-6">
            <input type="text"
                   ...
                   placeholder="{{ __('Buscar por familia, jefe o lote...') }}"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-neutral-200 dark:border-neutral-700">
                        <th class="text-left py-3 px-4 font-semibold">{{ __('Familia') }}</th>
                        <th class="text-left py-3 px-4 font-semibold">{{ __('Jefe de Familia') }}</th>
                ...
                <tbody>
                    ...
                    <tr x-show="matches($el.dataset.filter)"
                        x-cloak
                        data-filter="{{ \Illuminate\Support\Str::lower($familia->nombre_familia.' '.$familia->jefeFamilia->ape_paterno.' '.$familia->jefeFamilia->ape_materno.' '.$familia->jefeFamilia->nombres.' '.$familia->jefeFamilia->dni.' '.($familia->lote?->numero_lote ?? '').' '.($familia->lote?->manzana ?? '')) }}"
                        class="border-b border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-800">
                        <td class="py-3 px-4">{{ $familia->nombre_familia }}</td>
                        <td class="py-3 px-4">{{ $familia->jefeFamilia->ape_paterno }} {{ $familia->jefeFamilia->ape_materno }}, {{ $familia->jefeFamilia->nombres }}</td>
                        <td class="py-3 px-4">{{ $familia->jefeFamilia->dni }}</td>
                        <td class="py-3 px-4">
                            {{ $familia->lote?->numero_lote ?? '-' }}
                            @if(!empty($familia->lote?->manzana))
                                <span class="text-xs text-neutral-500">(Mz {{ $familia->lote->manzana }})</span>
                            @endif
                        </td>
                        <td class="py-3 px-4">
                            <div class="flex gap-2">
                                <button wire:click="openEdit({{ $familia->id }})"
                                        type="button"
                                        title="{{ __('Editar') }}"
                                ...
                                <button onclick="
                                    Swal.fire({
                                        ...
                                        text: '¿Estás seguro de eliminar esta familia?',
                                        icon: 'warning',
        ...
        <div wire:click.stop
            ...
            <div class="flex items-center justify-between mb-4">
                <h3 id="modal-title" class="text-xl font-bold">
                    {{ $editingId ? __('Editar Familia') : __('Nueva Familia') }}
                </h3>
            ...
            <form wire:submit="save" class="space-y-4">
                <div>
                    <label for="nombre_familia" class="block text-sm font-medium mb-2">{{ __('Nombre de Familia') }} *</label>
                    <input type="text"
                           id="nombre_familia"
                           wire:model="form.nombre_familia"
                           ...
                           required>
                    @error('form.nombre_familia')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>

                <div>
                    <label for="jefe_search" class="block text-sm font-medium mb-2">{{ __('Buscar Jefe por DNI') }} *</label>
                    ...
                    @if($showSearchResults && empty($selectedJefeNombre))
                    <div class="mt-2 px-3 py-2 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                        {{ __('No se encontró ciudadano con ese DNI') }}
                    </div>
                    @endif

                    @error('form.jefe_familia_id')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>

                <div>
                    <label for="jefe_nombre" class="block text-sm font-medium mb-2">{{ __('Nombre Completo') }}</label>
                    <div class="relative">
                        <input type="text"
                               id="jefe_nombre"
                               value="{{ $selectedJefeNombre }}"
                               readonly
                               placeholder="{{ __('Se llenará automáticamente') }}"
                               class="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-neutral-100 dark:bg-neutral-700 dark:text-white text-neutral-600 dark:text-neutral-300 cursor-not-allowed {{ $form['jefe_familia_id'] ? 'pr-10' : '' }}">
                        @if($form['jefe_familia_id'])
                        <button type="button"
                                wire:click="limpiarJefeFamilia"
                ...
                <div>
                    <label for="lote_id" class="block text-sm font-medium mb-2">{{ __('Lote') }}</label>
                    <select id="lote_id"
                            wire:model="form.lote_id"
                            class="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-neutral-700 dark:text-white">
                        <option value="">{{ __('Sin lote asignado') }}</option>
                        @foreach($lotes as $id => $nombre)
